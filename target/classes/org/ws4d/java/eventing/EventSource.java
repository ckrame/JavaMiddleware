 ...
 * <p>
 * The JMEDS framework supports two types of <a href="http://www.w3.org/TR/wsdl">WSDL 1.1</a> and <a href="http://www.w3.org/Submission/WS-Eventing/">WS-Eventing</a> compliant events: <strong>notifications</strong> and <strong>solicit-response</strong> operations. While the first ones represent one-way messages sent from the event source to its subscribers, the later additionally includes response messages sent back from the subscribers to the source.
 * <p>
 * Clients willing to receive notifications from this event source may simply {@link #subscribe(EventListener, long, DataStructure, CredentialInfo)
 * subscribe} to it. A subscription can be defined to expire after a certain amount of time ( <em>duration</em>) or it may last &quot;forever&quot;, i.e. until either the event source or the subscriber explicitly cancels it or it terminates due to shutdown or missing network reachability.
 * </p>
 */
public interface EventSource extends OperationDescription {
	 ...
	 * <p>
	 * This method will generate auto-bindings when subscribing to an event source which belongs to a <em>remotely</em> deployed service. This/These binding(s) will be used to open a local {@link EventSink event sink} and listen for incoming notifications.
	 * </p>
	 * <p>
	 * This method returns a {@link ClientSubscription} instance, which can be used to explore and manage the state of the subscription (e.g. renew or unsubscribe, etc.).
	 * </p>
	 * 
	 * @param client the client to which notification messages from this event
	 *            source are to be sent
	 * @param duration time until subscription expires in milliseconds; <code>0</code> means subscription never expires
	 * @return a client subscription allowing management of the subscription
	 *         state
	 * @throws CommunicationException if this event source belongs to a remote
	 *             service and sending the subscription to it timed out
	 ...
	 * <p>
	 * When subscribing to an event source, which belongs to a <em>remotely</em> deployed service, it is important to include at least one {@link CommunicationBinding} within the argument <code>bindings</code>. This/These binding(s) will be used to open a local {@link EventSink event
	 ...
	 * @param duration time until subscription expires in milliseconds; <code>0</code> means subscription never expires
	 * @param bindings a data structure consisting of one or more {@link CommunicationBinding} instances; those are used to bind
	 ...
	 * @throws EventingException if an error occurs during subscription
	 */
	public ClientSubscription subscribe(EventListener eventListener, long duration, DataStructure bindings, CredentialInfo credentialInfo) throws EventingException, IOException, CommunicationException;
