 ...
 * <p>
 * To receive events/ solicit responses, the event sink needs to have at least one {@link CommunicationBinding}. A communication binding can be added by {@link #addBinding(CommunicationBinding)} to the event sink. Afterwards the event sink must be opened by {@link #open()}, so that all bindings are bound to their {@link CommunicationManager}s.
 * </p>
 * <strong>Important</strong> Subscribing to events is done via {@link Service#subscribe(EventSink, String, org.ws4d.java.types.URISet, long, CredentialInfo)} .
 */
public interface EventSink extends Bindable {
	 ...
	 */
	public ParameterValue receiveLocalEvent(String clientSubscriptionId, URI actionUri, ParameterValue outputParameter, CredentialInfo credentialInfo);
	 ...
	 * Subscribing to an event is done by {@link Service#subscribe(EventSink, String, org.ws4d.java.types.URISet, long, CredentialInfo)} .
	 * 
	...
	/**
	 * @param clientSubId
	 * @return the client subscription with the given ID that has been removed
	 */
	 ...
	 * @param delivery
	 * @param useReferenceParameterMode
	 * @return
	 */
