...
public class WSDL implements Resource {
	...
	public static final ContentType	CONTENT_TYPE				= new ContentType("WSDL", "WSDL", "wsdl");
	...
	public static WSDL parse(URI fromUri, CredentialInfo credentialInfo, String comManId) throws XmlPullParserException, IOException {
		return parse(fromUri, credentialInfo, true, comManId);
	}

	public static WSDL parse(URI fromUri, CredentialInfo credentialInfo, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException {
		ResourceLoader rl = JMEDSFramework.getResourceAsStream(fromUri, credentialInfo, comManId);
		InputStream in;
		if (rl != null && (in = rl.getInputStream()) != null) {
			try {
				WSDL w = parse(in, fromUri, credentialInfo, loadReferencedFiles, comManId);

				MonitorStreamFactory monFac = JMEDSFramework.getMonitorStreamFactory();

				if (monFac != null) {
					ConnectionInfo ci = rl.getConnectionInfo();
					if (ci != null) {
						MonitoringContext context = monFac.getMonitoringContextIn(ci.getConnectionId());
						monFac.receiveResourceResponse(ci.getConnectionId(), context, w);
					}
				}
			...
			throw new IOException("Unable to read WSDL from URI: " + fromUri);
		}
	}

	public static WSDL parse(InputStream in, CredentialInfo credentialInfo, String comManId) throws XmlPullParserException, IOException {
		return parse(in, credentialInfo, true, comManId);
	}

	public static WSDL parse(InputStream in, CredentialInfo credentialInfo, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException {
		return parse(in, null, credentialInfo, loadReferencedFiles, comManId);
	}

	public static WSDL parse(InputStream in, URI fromUri, CredentialInfo credentialInfo, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException {
		return CommunicationManagerRegistry.getCommunicationManager(comManId).newDescriptionParser().parse(in, fromUri, credentialInfo, null, loadReferencedFiles, comManId);
	}

	public static WSDL parse(XmlPullParser parser, CredentialInfo credentialInfo, String comManId) throws XmlPullParserException, IOException {
		return parse(parser, credentialInfo, true, comManId);
	}

	public static WSDL parse(XmlPullParser parser, CredentialInfo credentialInfo, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException {
		return CommunicationManagerRegistry.getCommunicationManager(comManId).newDescriptionParser().parse(parser, null, credentialInfo, null, loadReferencedFiles, comManId);
	}
	...
	public HashMap getHeaderFields() {
		...
		map.put(HTTP_HEADER_LAST_MODIFIED, StringUtil.getHTTPDate(lastMod));
		return map;
	}
	 ...
	 * @return the Schema instance with the given target namespace or <code>null</code>
	 */
	 ...
	 *         this WSDL or any of its recursively imported WSDLs or <code>null</code>
	 */
	...
	public void serialize(OutputStream out, String comManId) throws IOException {
		CommunicationManagerRegistry.getCommunicationManager(comManId).newDescriptionSerializer().serialize(this, out);
	}

	/**
	 * @return the default namespaces.
	 */
	...
	public static void addDefaultNamespaceAndPrefix(String namespace, String prefix) {
		DEFAULT_NAMESPACES.put(namespace, prefix);
	}
