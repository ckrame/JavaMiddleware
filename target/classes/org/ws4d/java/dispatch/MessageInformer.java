 ...
 * messages crossing the JMEDS framework. This is especially useful for clients
 * interested in receiving specific types of messages or for traffic analyzers
 * willing to get notified about each message sent or received.
 * <p>
 * When registering, a listener can declare its interest for certain messages by passing a {@link MessageSelector} to the method {@link #addMessageListener(MessageListener, MessageSelector)}. The <code>MessageSelector</code>'s {@link MessageSelector#matches(Message)} method is used to determine whether a given message matches this interest or not. The JMEDS framework provides two standard implementations of <code>MessageSelector</code>: {@link AllMessageSelector}, which simply marks every message as interesting, and {@link DefaultMessageSelector} allowing discrimination based on a message's type and/or its target endpoint address (i.e. the WS-Addressing [destination] property of the message).
 * </p>
 */
public class MessageInformer {
	 ...
	 * JMEDS framework that match the specified message selector. If <code>sel</code> is <code>null</code>, the listener will be registered
	...
	public void forwardMessage(Message msg, ConnectionInfo connectionInfo, AttributedURI optionalMessageId) {
		forwardMessageInternal(msg, connectionInfo, optionalMessageId, true);
	}

	private void forwardMessageInternal(Message msg, ConnectionInfo connectionInfo, AttributedURI optionalMessageId, boolean notifyRunner) {
		if (msg == null) {
			return;
		}
		try {
			queuedMessagesLock.exclusiveLock();
			queuedMessages.add(new MessageEntry(msg, connectionInfo, optionalMessageId));
		} finally {
	...
	private void deliverMessages() {
		try {
			...
			// (INGO) potentially DANGEROUS because the variable size in class
			// List is not volatile
		...
		try {
			...
			for (Iterator it2 = listeners.entrySet().iterator(); it2.hasNext();) {
				...
				if (sel.matches(msg)) {
					...
					if (msg.isInbound()) {
						listener.receivedInboundMessage(msg, messageEntry.connectionInfo, messageEntry.optionalMessageId);
					} else {
						listener.receivedOutboundMessage(msg, messageEntry.connectionInfo, messageEntry.optionalMessageId);
					}
				}
			}
		} finally {
	...
	private static final class MessageEntry {
		...
		final AttributedURI		optionalMessageId;

		/**
		 * @param message
		 * @param connectionInfo
		 */
		MessageEntry(Message message, ConnectionInfo connectionInfo, AttributedURI optionalMessageId) {
			super();
