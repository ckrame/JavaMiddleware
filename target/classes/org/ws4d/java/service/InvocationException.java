 ...
 * within such an exception allows a caller to determine the {@link #getAction() type} of fault and extract any additional {@link #getDetail() user-defined data} attached to it.
 */
public class InvocationException extends Exception {
	 ...
	 */
	public InvocationException(Fault fault, QName code, ParameterValue params) {
		this(fault, code, DECLARED_FAULT_REASON + fault.getName(), params);
	}

	/**
	 * Creates a new instance representing the specified <code>fault</code> with
	 * the given reason and carrying the user-defined data <code>params</code> as additional fault details.
	 * 
	 * @param fault the fault this exception represents
	 * @param reason the fault reason
	 * @param params user-defined additional data about the fault
	 */
	public InvocationException(Fault fault, QName code, String reason, ParameterValue params) {
		this(fault.getAction(), code, WS4DConstants.WS4D_FAULT_DECLARED, createReasonFromString(reason), params);
	}
	 ...
	 * @param params user-defined additional data about the fault
	 */
	public InvocationException(Fault fault, QName code, DataStructure reason, ParameterValue params) {
		this(fault.getAction(), code, WS4DConstants.WS4D_FAULT_DECLARED, reason, params);
	}
	...
	public InvocationException(FaultMessage fault) {
		this(MessageConstants.getMessageNameForType(fault.getType()), fault.getCode(), fault.getSubcode(), fault.getReason(), fault.getDetail());
	}
	 ...
	 * @param reason the fualt's reason, a data structure of {@link LocalizedString} , each for a different language or
	 *            locale instances
	 * @param detail additional user-defined data further describing the fault
	 */
	public InvocationException(String action, QName code, QName subcode, DataStructure reason, ParameterValue detail) {
		super();
		this.action = action;
		this.code = code;
