 ...
 * just like exceptions may be declared on Java methods. A fault has a {@link #getName() name}, which must not be <code>null</code> and unique
 * within the scope of the surrounding operation/event, and an {@link #getAction() action URI} corresponding to the <em>wsa:Action</em> property of the fault message within which the fault is sent . The action URI
 ...
 * omit specifying the action URI explicitly as the JMEDS framework will provide
 * an auto-generated one in case it was not set prior to adding the fault to an
 * operation/event.
 * <p>
 * A <code>Fault</code> can also carry user-defined data, just like exceptions can be designed to encapsulate additional state within their instance variables. Within the <code>Fault</code>, this state is described by means of its associated {@link #getElement() element}.
 * </p>
 * <p>
 * It is important to notice that modifications to a fault are not possible after it has been added to an operation. Calling any of the setter methods at this time will result in <code>IllegalStateException</code>s being thrown.
 * </p>
 */
public class Fault extends AttributableSupport {
	 ...
	 * an auto-generated {@link #getAction() action} when it is {@link OperationCommons#addFault(Fault) added} to an operation or event.
	 * 
	 ...
	 * <p>
	 * As <code>Faults</code> are always required to have an action URI associated to them, one will be automatically generated when adding this <code>Fault</code> to an {@link Operation operation} or {@link DefaultEventSource event}, if it was not specified explicitly.
	 * 
	 ...
	 * only <strong>before</strong> this <code>Fault</code> is added to an {@link Operation operation} or {@link DefaultEventSource event}.
	 * 
	 ...
	 * of any user-defined data the fault may contain. It can be used to {@link #createValue() create} a suitable {@link ParameterValue} container
	 ...
	 * added this <code>Fault</code> to an {@link Operation operation} or {@link DefaultEventSource event} will result in an <code>IllegalStateException</code> being thrown.
	 * 
	 ...
	 * fault's element. Technically equivalent to <code>ParameterValue.createElementValue(getElement())</code>.
	 * 
	...
	/**
	 * A shorthand method for creating a suitable data container for this
	 * fault's element. Technically equivalent to <code>ParameterValue.createElementValue(getElement(), instanceType)</code> .
	 * <p>
	 ...
	 * @see #getElement()
	 */
	public ParameterValue createValue(Type instanceType) {
		return ParameterValueManagement.createElementValue(getElement(), instanceType);
	}
}
