 ...
 * XMPULL V1 API. The name of the actual factory class will be determined
 * according to several parameters. It works in a similar way to JAXP but is
 * tailored to work in J2ME environments (no access to system properties or the
 * file system) so the name of parser class factory to use and its class used
 * for loading (no class loader - on J2ME no access to context class loaders)
 * must be passed explicitly. If no name of parser factory was passed (or is
 * null) it will try to find name by searching in CLASSPATH for
 * META-INF/services/org.xmlpull.v1.XmlPullParserFactory resource which should
 * contain a comma separated list of class names of factories or parsers to try
 * (in order from left to the right). If none are found, it will throw an
 * exception. <br />
 * <strong>NOTE:</strong>In J2SE or J2EE environments, you may want to use <code>newInstance(property, classLoaderCtx)</code> where first argument is <code>System.getProperty(XmlPullParserFactory.PROPERTY_NAME)</code> and
...
public class XmlPullParserFactory {
	...
	public static final String	PROPERTY_NAME	= "org.ws4d.java.xmlpull.v1.XmlPullParserFactory";

	private static final String	RESOURCE_NAME	= "/services/" + PROPERTY_NAME;
	...
	/**
	 * Sets the features to be set when XML Pull Parser is created by this
	 * factory.
	 * <p>
	...
	/**
	 * Returns the current value of the feature with this given name.
	 * <p>
	 * <b>NOTE:</b> factory features are not used for XML Serializer.
	 * 
	 ...
	 * which are namespace aware (it simply sets feature
	...
	public XmlPullParser newPullParser() throws XmlPullParserException {
		...
		final SimpleStringBuilder issues = Toolkit.getInstance().createSimpleStringBuilder();
	...
	/**
	 * Creates a new instance of a PullParserFactory that can be used to create
	...
	public static XmlPullParserFactory newInstance(String classNames, Class context) throws XmlPullParserException {
		...
		if (classNames == null || classNames.length() == 0 || "DEFAULT".equals(classNames)) {
			try {
				...
				final SimpleStringBuilder sb = Toolkit.getInstance().createSimpleStringBuilder();
