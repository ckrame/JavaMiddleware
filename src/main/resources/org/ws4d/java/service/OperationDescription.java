...
public interface OperationDescription {
	 ...
	 * message. Will return <code>null</code> only if this operation's {@link #getType() transmission type} is {@link WSDLOperation#TYPE_NOTIFICATION}, i.e. the operation doesn't have
	 * any input. Otherwise, if no input action was explicitly {@link OperationCommons#setInputAction(String) set}, this method will
	 ...
	 * doesn't have any input (i.e. its {@link #getType() transmission type} is {@link WSDLOperation#TYPE_NOTIFICATION}, <code>null</code> is returned.
	 * Otherwise, if no input name was {@link OperationCommons#setInputName(String) set} previously, a default
	 ...
	 * message. Will return <code>null</code> only if this operation's {@link #getType() transmission type} is {@link WSDLOperation#TYPE_ONE_WAY}, i.e. the operation doesn't have any
	 * output. Otherwise, if no output action is explicitly {@link OperationCommons#setOutputAction(String) set}, this method will
	 * generate a default one according to the rules of <a
	 * href="http://www.w3.org/TR/ws-addr-wsdl/">WS-Addressing 1.0 - WSDL
	 * Binding</a>.
	 * 
	 ...
	 * doesn't have any output (i.e. its {@link #getType() transmission type} is {@link WSDLOperation#TYPE_ONE_WAY}, <code>null</code> is returned.
	 * Otherwise, if no output name has been previously {@link OperationCommons#setOutputName(String) set}, a default output name
	...
	/**
	 * Returns the service to which this operation is associated. May return <code>null</code>, if this operation has not yet been added to a
	 * particular service.
	 * 
	 ...
	 * equivalent to <code>ParameterValueManagement.createElementValue(Element element)</code> .
	 * 
	 ...
	 * equivalent to <code>ParameterValueManagement.createElementValue(Element);</code>.
	 * 
