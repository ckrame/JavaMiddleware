package org.ws4d.java.xmlpull.mxp1_serializer;
...
public class MXSerializer implements XmlSerializer {
	...
	private String generatePrefix(String namespace) {
		becauseAlreadyExists: while (true) {
			...
			for (int i = namespaceEnd - 1; i >= 0; --i) {
				if (prefix.equals(namespacePrefix[i])) {
					continue becauseAlreadyExists; // prefix is already declared - generate new and
					// try again
				}
			}
			// declare prefix
	...
	protected static final String printable(String s) {
		...
		SimpleStringBuilder retval = Toolkit.getInstance().createSimpleStringBuilder(s.length() + 16);
		retval.append("'");
	...
	private static void addPrintable(SimpleStringBuilder retval, char ch) {
		switch (ch) {
			case '\b':
				retval.append("\\b");
				break;
