...
public class AllContainer extends ElementContainer {
	...
	public String toString() {
		...
		return "AllContainer [ own=" + elementCount + ", inherit=" + (all - elementCount) + ", all=" + all + ", min=" + min + ", max=" + max + ", container=" + container + " ]";
	}
	...
	public int getContainerType() {
		return ComplexType.CONTAINER_ALL;
	}
	...
	void serialize(XmlSerializer serializer, Schema schema) throws IOException {
		serializer.startTag(XMLSCHEMA_NAMESPACE, TAG_ALL);
		serialize0(serializer, schema);
		serializer.endTag(XMLSCHEMA_NAMESPACE, TAG_ALL);
	}
