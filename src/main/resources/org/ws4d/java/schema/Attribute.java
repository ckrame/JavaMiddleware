...
public class Attribute extends Reference {
	...
	static final Attribute createAttribute(ElementParser parser, String targetNamespace, Schema schema) throws XmlPullParserException, IOException, SchemaException {
		...
		if (aName != null) {
			...
			if (index != -1) {
				String prefix = aName.substring(0, index);
		...
		if (aName == null) {
			...
			a = new Attribute(new QName(aName, (namespace != null) ? namespace : targetNamespace));
		}
	...
	public Type getType() {
		if (reference != null) {
			return ((Attribute) reference).getType();
		}
		if (type == null) {
	...
	void serialize(XmlSerializer serializer, Schema schema) throws IOException {
		...
		if (name != null) {
			...
			if (!(prefix == null || "".equals(prefix))) {
				...
				serializer.attribute(null, ATTRIBUTE_NAME, name.getLocalPartPrefixed());
			} else {
		...
		if (type != null) {
			...
			if (typeName == null) {
				...
				String prefix = serializer.getPrefix(typeName.getNamespace(), true);
				if (!(prefix == null || "".equals(prefix))) {
			...
			String prefix = serializer.getPrefix(refName.getNamespace(), true);
			if (!(prefix == null || "".equals(prefix))) {
