...
public class AttachmentProperties implements PropertiesHandler {
	 ...
	 * <strong>MUST</strong> be less than or equal to {@link #PROP_MAX_ATTACHMENT_SIZE}.<br />
	 * Default: 65536
	 */
	 ...
	 * support, see InputStreamAttachment and {@link AttachmentFactory#getStreamingMediaTypes()}. <br/>
	 * Default: 128
	 */
	...
	public void setProperties(PropertyHeader header, Property property) {
		if (Properties.HEADER_SUBSECTION_ATTACHMENT.equals(header)) {
			if (PROP_ATTACHMENT_STORE_POLICY.equals(property.key)) {
				...
				maxAttachmentSize = Long.parseLong(property.value.trim());
			} else if (PROP_MAX_MEM_BUFFER_SIZE.equals(property.key)) {
				maxMemBufferSize = Long.parseLong(property.value.trim());
