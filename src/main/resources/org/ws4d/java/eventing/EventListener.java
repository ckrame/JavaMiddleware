...
public interface EventListener {
	 ...
	 * notifications from a service. The supplied data structure <code>bindings</code> is supposed to contain at least one {@link CommunicationBinding} instance denoting a local transport address,
	 * at which incoming notifications shall be delivered to. The EventSink is
	 * associated with the EventListener.
	 * 
	 * @param bindings a data structure of {@link CommunicationBinding} instances to expose the created event sink over
	 ...
	 * events.
	 * <p>
	 * In case the received event corresponds to a {@link WSDLOperation#TYPE_SOLICIT_RESPONSE} operation, the response to send back to the event source is taken from the return value of this method.
	 * </p>
	 * 
	 ...
	 * @param subscriptionEndType type of the subscription end
	 */
