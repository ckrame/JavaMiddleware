...
public class AppSequenceTracker {
	...
	public AppSequenceTracker(AppSequenceTracker oldTracker) {
		oldestSequenceIdIndex = oldTracker.oldestSequenceIdIndex;
		latestInstanceId = oldTracker.latestInstanceId;

		if (oldTracker.sequenceIds == null) {
			sequenceIds = null;
		} else {
			sequenceIds = new String[oldTracker.sequenceIds.length];
			System.arraycopy(sequenceIds, 0, oldTracker.sequenceIds, 0, oldTracker.sequenceIds.length);
		}

		if (oldTracker.latestMessageNumbers == null) {
			latestMessageNumbers = null;
		} else {
			latestMessageNumbers = new long[oldTracker.latestMessageNumbers.length];
			System.arraycopy(latestMessageNumbers, 0, oldTracker.latestMessageNumbers, 0, oldTracker.latestMessageNumbers.length);
		}
		latestNullMessageNumber = oldTracker.latestNullMessageNumber;
	}
	 ...
	 */
	public synchronized boolean checkAndUpdate(AppSequence appSeq, boolean acceptEqual) {
		// null check, THX @Stefan Schlichting
		if (appSeq == null) {
		...
		if (sequenceId == null) {
			if (acceptEqual) {
				if (messageNumber < latestNullMessageNumber) {
					return false;
				}
			} else {
				if (messageNumber <= latestNullMessageNumber) {
		...
		for (; i < sequenceIds.length; i++) {
			...
			if (sequenceIds[i].equals(sequenceId)) {
				if (acceptEqual) {
					if (messageNumber < latestMessageNumbers[i]) {
						return false;
					}
				} else {
					if (messageNumber <= latestMessageNumbers[i]) {
		...
		if (i >= sequenceIds.length) {
			if (i < MAX_NUMBER_OF_SEQUENCEIDS) {
				...
				if (oldestSequenceIdIndex == sequenceIds.length) {
					oldestSequenceIdIndex = 0;
					i = 0;
				}
			}
