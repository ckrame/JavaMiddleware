...
public interface IncomingFileAttachment extends IncomingAttachment {
	 ...
	 * is only legal for attachments of {@link #getType() type} {@link #FILE_ATTACHMENT}. For any other types, it will throw an {@link AttachmentException}.
	 * 
	 ...
	 * "C:/folder/file.gif"). This method can only be used for attachments of {@link #getType() type} {@link #FILE_ATTACHMENT}. In any other cases it
	 ...
	 *            file to
	 * @return <code>true</code>, if the file was moved/renamed successfully, <code>false</code> otherwise
	 * @throws AttachmentException if attachment processing is not supported
	 *             within the current runtime or obtaining the attachment failed
	 *             for any reason
	 */
	 ...
	 * type} {@link #FILE_ATTACHMENT}, as - if it returns <code>true</code> - this denotes that the {@link #getAbsoluteFilename() file} the attachment points to is the original one. That is, when attempting to {@link #move(String)} a file attachment, this allows to distinguish whether the original file would be moved to a new location or whether simply the attachment file will be moved out of the local attachment store (used for caching incoming attachments) to a place outside of it.
	 * 
