...
/**
 * This class is the common base for both {@link Operation}s and {@link DefaultEventSource}s within the JMEDS framework. It contains all
 * common properties for WSDL 1.1 compliant operations. It is further used to
 * declare an operation's or event's {@link #getInput() input} and {@link #getOutput() output} parameters as well as any {@link #getFaults()
 * faults} that may occur on invocation. Overall in this class' documentation,
 * the term <em>&quot;operation&quot;</em> is used to mean either an actual {@link Operation operation} or an {@link DefaultEventSource event}.
 * <p>
 * <strong>Note:</strong> According to <a href="http://www.w3.org/TR/wsdl">WSDL 1.1 Specification</a>, an operation's {@link #getName() name} is not required to be unique within the scope of its containing port type in order to support overloading. However, when overloading operations, the combination of each one's {@link #getName() name}, {@link #getInputName() input name} and {@link #getOutputName() output name} must be unique in order to avoid name clashes.
 * </p>
 */
public abstract class OperationCommons extends AttributableSupport implements OperationDescription {
	...
	// Changed 2010-08-11 SSch Changed to final, Thx to Stefan Schlichting
	/* The WSDL PortType of the Operation */
	private final QName		portType;

	private String			quallifiedActionName	= null;
	...
	public String getNameQuallified() {
		if (quallifiedActionName == null) {
			quallifiedActionName = getPortType().getNamespace() + QName.NAMESPACE_SEPARATOR + getName();
		}

		return quallifiedActionName;
	}
	 ...
	 * returned is one of {@link WSDLOperation#TYPE_ONE_WAY}, {@link WSDLOperation#TYPE_REQUEST_RESPONSE}, {@link WSDLOperation#TYPE_NOTIFICATION} and {@link WSDLOperation#TYPE_SOLICIT_RESPONSE}.
	 * 
	 ...
	 * have any input (i.e. its {@link #getType() transmission type} is {@link WSDLOperation#TYPE_NOTIFICATION}, <code>null</code> is returned.
	 ...
	 * operation. Will return <code>null</code> only if this operation's {@link #getType() transmission type} is {@link WSDLOperation#TYPE_NOTIFICATION}, i.e. the operation doesn't have
	 * any input. Otherwise, if no input action was explicitly {@link #setInputAction(String) set}, this method will generate a default
	 ...
	 * have any output (i.e. its {@link #getType() transmission type} is {@link WSDLOperation#TYPE_ONE_WAY}, <code>null</code> is returned.
	 * Otherwise, if no output name was previously {@link #setOutputName(String) set} a default output name is generated
	 ...
	 * this operation. Will return <code>null</code> only if this operation's {@link #getType() transmission type} is {@link WSDLOperation#TYPE_ONE_WAY}, i.e. the operation doesn't have any
	 * output. Otherwise, if no output action is explicitly {@link #setInputAction(String) set}, this method will generate a default
	 * one according to the rules of <a
	 * href="http://www.w3.org/TR/ws-addr-wsdl/">WS-Addressing 1.0 - WSDL
	 * Binding</a>.
	 * 
	 ...
	 * used to {@link #createInputValue() create} a suitable {@link ParameterValue} container for the actual values when sending
	 ...
	 * wrapper element.
	 * <p>
	 * <strong>NOTE:</strong> Using this method is only possible when the input element for this operation was <em>NOT</em> previously set by a call to {@link #setInput(Element)}. Otherwise, this method will throw a {@link WS4DIllegalStateException}.
	 * </p>
	 * 
	 * @param parameterName the name of the input parameter to add
	 * @param type the type for the new input parameter
	 * @throws WS4DIllegalStateException if an input element was previously {@link #setInput(Element) set} on this operation
	 */
	 ...
	 * used to {@link #createOutputValue() create} a suitable {@link ParameterValue} container for the actual values an operation
	 * returns.
	 * 
	 ...
	 * wrapper element.
	 * <p>
	 * <strong>NOTE:</strong> Using this method is only possible when the output element for this operation was <em>NOT</em> previously set by a call to {@link #setOutput(Element)}. Otherwise, this method will throw a {@link WS4DIllegalStateException}.
	 * </p>
	 * 
	 * @param parameterName the name of the output parameter to add
	 * @param type the type for the new output parameter
	 * @throws WS4DIllegalStateException if an output element was previously {@link #setOutput(Element) set} on this operation
	 */
	 ...
	 * <p>
	 * A fault on a web service operation corresponds to a checked declared exception on a Java method. If an operation's {@link Operation#invoke(ParameterValue) invocation} causes a fault, the invoking client will receive an {@link InvocationException} providing additional information about this fault.
	 * </p>
	 * 
	 ...
	 * equivalent to <code>ParameterValue.createElementValue(getOutput());</code>
	 * 
	 * @return a parameter container suitable for this operation's output
	 * @see #getOutput()
	 * @see ParameterValueManagement#createElementValue(Element)
	 */
	public ParameterValue createOutputValue() {
		return ParameterValueManagement.createElementValue(getOutput());
	}

	/**
	 * This is a shorthand method for creating a {@link ParameterValue parameter
	 * value} container for the output of this operation. It is semantically
	 * equivalent to <code>ParameterValue.createElementValue(getOutput(), instanceType);</code>
	 * <p>
	 * It is intended to be used with instanceType being a subtype of the type defined for output value.
	 * 
	 * @param instanceType type of the ParameterValue returned by this method
	 * @return a parameter container suitable for this operation's output
	 * @see #getOutput()
	 * @see ParameterValueManagement#createElementValue(Element)
	 */
	public ParameterValue createOutputValue(Type instanceType) {
		return ParameterValueManagement.createElementValue(getOutput(), instanceType);
	}
	...
	public ParameterValue createInputValue() {
		return ParameterValueManagement.createElementValue(getInput());
	}

	/**
	 * This is a shorthand method for creating a {@link ParameterValue
	 * ParameterValue} container for the input of this operation. It is
	 * semantically equivalent to <code>ParameterValue.createElementValue(getInput(), instanceType);</code>
	 * <p>
	 ...
	 * @see ParameterValueManagement#createElementValue(Element)
	 */
	public ParameterValue createInputValue(Type instanceType) {
		return ParameterValueManagement.createElementValue(getInput(), instanceType);
	}
	 ...
	 * @see #getFault(String)
	 */
	public ParameterValue createFaultValue(String faultName, Type instanceType) {
		Fault fault = getFault(faultName);
		if (fault == null) {
			throw new NoSuchElementException("unknown fault: " + faultName);
		}
	...
	/**
	 * Returns the service to which this operation is associated. May return <code>null</code>, if this operation has yet not been added to a
	 * particular service.
	 * 
	...
	/**
	 * Returns the value of the input attribute with the given <code>name</code> or <code>null</code> if this attribute is not available (or if its value
	...
	/**
	 * Sets the <code>value</code> for the input attribute with the specified <code>name</code>. Throws a <code>java.lang.IllegalArgumentException</code> in case <code>name</code> is <code>null</code>.
	 * 
	 * @param attributeName the name of the input attribute to set, must not be <code>null</code>
	 * @param value the value to which to set the named input attribute (may be <code>null</code>
	...
	/**
	 * Sets all input attributes at once to those contained within argument <code>attributes</code>. Note that depending on the actual implementation
	 ...
	 * to this map should be made with care . This method throws a <code>java.lang.IllegalArgumentException</code> in case <code>attributes</code> is <code>null</code>.
	 * 
	...
	/**
	 * Returns the value of the output attribute with the given <code>name</code> or <code>null</code>, if this attribute is not
	 * available (or if its value is actually explicitly set to <code>null</code>).
	 * 
	...
	/**
	 * Sets the <code>value</code> of the output attribute with the specified <code>name</code>. Throws a <code>java.lang.IllegalArgumentException</code> in case <code>name</code> is <code>null</code>.
	 * 
	 * @param attributeName the name of the output attribute to set, must not be <code>null</code>
	 * @param value the value to set the named output attribute to (may be <code>null</code>
	 * @throws IllegalArgumentException if <code>name</code> is <code>null</code>
	 */
	...
	/**
	 * Sets all output attributes at once to those contained within argument <code>attributes</code>. Note that depending on the actual implementation
	 * it is possible that the map <code>attributes</code> points at may be used
	 ...
	 * care after passing it to this method. This method throws a <code>java.lang.IllegalArgumentException</code> in case <code>attributes</code> is <code>null</code>.
	 * 
	...
	/**
	 * Returns the value of the fault attribute with the given <code>name</code> for the fault with the specified unique <code>faultName</code> or <code>null</code>, either if this attribute is not available (or if its
	 * value is actually explicitly set to <code>null</code>).
	 * <p>
	 * This method throws a <code>java.lang.IllegalArgumentException</code> if a fault with the given <code>faultName</code> is not found within this instance.
	 * </p>
	 * 
	 ...
	 * @throws IllegalArgumentException if no fault with the given <code>faultName</code> is found
	 */
	...
	/**
	 * Sets the <code>value</code> for the fault attribute with the specified <code>name</code> of the fault with the given unique <code>faultName</code>. Throws a <code>java.lang.IllegalArgumentException</code> in case there is no fault
	 * with the given <code>faultName</code> within this instance or if <code>name</code> is <code>null</code>.
	 * 
	 * @param faultName the unique name of the fault within the scope of this
	 *            instance, see {@link #getFault(String)}
	 * @param attributeName the name of the fault attribute to set, must not be <code>null</code>
	 * @param value the value to set the named fault attribute to (may be <code>null</code>
	 * @throws IllegalArgumentException if there is no fault with the given <code>faultName</code> within this instance or if <code>name</code> is <code>null</code>
	 */
	...
	/**
	 * Sets at once all fault attributes of the fault with unique <code>faultName</code> to those contained within argument <code>attributes</code>. Note that depending on the actual
	 * implementation, it is possible that the map <code>attributes</code> points at may be used for the actual internal storage of the fault
	 ...
	 * @throws IllegalArgumentException if no fault with the given <code>faultName</code> is found or if <code>attributes</code> is <code>null</code>
	 */
	 ...
	 * attribute set for the fault with the specified unique <code>faultName</code>. Returns <code>false</code> in any other case,
	 ...
	 * <p>
	 * <h3>Notice</h3> Do <strong>not</strong> add custom types with the same name as the types used for input or output definitions. This may overwrite the normal definitions.
	 * </p>
	 * <p>
	 * The added custom type <strong>MUST</strong> have a valid name. If no name is set, this method will throw an {@link RuntimeException}.
	 * </p>
	 * 
	...
	private String buildActionName(String ioTypeName, String delim) {
		SimpleStringBuilder text = Toolkit.getInstance().createSimpleStringBuilder();
