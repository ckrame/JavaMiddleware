...
public class Base64Util {
		...
		try {
			...
			if (numBytes >= 0) {
				...
				while ((numBytes = bis.read(buffer, length, BLOCK_SIZE)) >= 0) {
					encodedData = encodedData.concat(new String(buffer, 0, length, Base64Util.PREFERRED_ENCODING));
					length += numBytes;
				}
			}
			...
			Log.error("Error encoding from InputStream");
	...
	/**
	 * A {@link Base64Util.InputStream} will read data from another <tt>java.io.InputStream</tt>, given in the constructor, and encode/decode
	...
	public static class InputStream extends java.io.InputStream {
		...
		private byte[]				buffer;		// Small buffer holding
		...
		private boolean				breakLines;	// Break lines at less than
		...
		public int available() throws IOException {
			// if ( encode ){
			// int len43 = in.available() * 4 / 3;
			// return ( len43
			// + ( (in.available() % 3) > 0? 4 : 0 )
			// + ( breakLines ? ( len43 / MAX_LINE_LENGTH ) : 0));
			// }
			// else {
			// return ( in.available() * 3 / 4 );// Upper limit on size of
			// output
			// }
			return 0;
	...
	/**
	 * A {@link Base64Util.OutputStream} will write data to another <tt>java.io.OutputStream</tt>, given in the constructor, and
	...
	public static class OutputStream extends java.io.OutputStream {
		...
		private byte[]					b4;				// Scratch used in a
