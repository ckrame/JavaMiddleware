...
/**
 * Used to {@link CommunicationManager#registerDevice(CommunicationBinding, IncomingMessageListener)
 * and CommunicationManager#registerDiscovery(int[],
 * org.ws4d.java.communication.DiscoveryBinding, IncomingMessageListener)
 * register} interest for incoming (request) messages of a certain type. When a
 * desired message is received, it is delivered to the matching <code>handle</code> method, e.g. {@link #handle(GetMessage, ConnectionInfo)} for a {@link GetMessage}.
 */
public interface IncomingMessageListener {
	 ...
	 * @param connectionInfo protocol (aka. technology) specific data associated
	 ...
	 * probe that matches, a corresponding non-empty {@link ProbeMatchesMessage} must be returned. Otherwise, if the multicast probe doesn't match, this
	 * method must return <code>null</code>. If the received probe is directed,
	 * then this method must always return a <code>ProbeMatchesMessage</code> which, in the case of a match, must be not empty.
	 * <p>
	 * When the incoming probe message is a directed one and for some reason this method fails (e.g. this implementation doesn't support handling of probe messages, unexpected probe content, etc.), a {@link SOAPException} describing the failure reason must be thrown. It will then be forwarded to the original sender of the probe.
	 * </p>
	 * 
	...
	/**
	 * Receives an incoming get message. Must return a valid {@link GetResponseMessage} if processing the get request succeeded. Must
	...
	/**
	 * Receives an incoming get metadata message. Must return a valid {@link GetMetadataResponseMessage} if processing the get metadata request
	...
	/**
	 * Receives an incoming subscribe message. Returns a suitable {@link SubscribeResponseMessage} or throws a {@link SOAPException} if
	...
	/**
	 * Receives an incoming get-status message (WS-Eventing). Returns a suitable {@link GetStatusResponseMessage} or throws a {@link SOAPException} if
	...
	/**
	 * Receives an incoming renew message (WS-Eventing). Returns a suitable {@link RenewResponseMessage} or throws a {@link SOAPException} if
	 * processing the incoming request fails for any reason.
	 * 
	 ...
	 * suitable {@link UnsubscribeResponseMessage} or throws a {@link SOAPException} if processing the incoming request fails for any
	 * reason.
	 * 
	 ...
	 * <p>
	 * <strong>NOTE:</strong> As this type of message is a simple one-way notification, this method neither has a return value, nor is declared to throw a {@link SOAPException} in case of a failure. That is, errors occurring during processing of this message must be silently ignored (this includes the case when this <code>IncomingMessageListener</code> instance doesn't process subscription end messages at all).
	 * 
	 ...
	 * request-response). For a one-way operation, this method must return <code>null</code>, whereas for a request-response operation, the returned {@link InvokeMessage} must contain a suitable response to the incoming
	 * request.
	 * <p>
	 * This method may throw a {@link SOAPException} under two distinct circumstances:
	 * <ul>
	 * <li>the web service operation to which the incoming request belongs declares one or more faults: in such a case, the thrown <code>SOAPException</code> is expected to correspond to either one of those declared faults</li>
	 * <li>processing the invoke request fails for any other reason, e.g. malformed content or this <code>IncomingMessageListener</code> instance being unable to handle invoke requests: then, the <code>SOAPException</code> must describe the exact failure reason in a way independent from the actual declaration of the involved web service operation</li>
	 * </ul>
	 * </p>
	 * 
	...
	/**
	 * Returns an operation for a given <strong>wsa:Action</strong>. <h3>Important</h3>
	 * <p>
	 * The result can be <code>null</code> for an implementation which does not use an operation at all.
	 * </p>
	 * 
