 ...
 * </ul>
 * <p>
 * Properties should be initialized before starting the {@link JMEDSFramework}.
 * </p>
 */
public final class Properties {
	...
	public final static PropertyHeader	HEADER_SECTION_BT						= new PropertyHeader(SECTION_BT);
	...
	// class for the BT communication manager

	public static final String			BT_PROPERTIES_HANDLER_CLASS				= "org.ws4d.java.configuration.BTProperties";
	...
	private static PropertiesHandler createHandlerInstance(String className) {
		try {
			Class handlerClass = Clazz.forName(className);
	...
	private String headerString(String[] s) {
		SimpleStringBuilder out = Toolkit.getInstance().createSimpleStringBuilder(s.length * 16);
	...
	private void initHandlers() {
		 ...
		 */
		if (JMEDSFramework.hasModule(JMEDSFramework.SERVICE_MODULE)) {
	...
	public void init(String filename) {
		try {
			InputStream fileStream = FileSystem.getInstance().readFile(filename);
			try {
				if (Log.isDebug()) {
					Log.debug("Properties: Try to load properties from: " + filename);
				}
				init(fileStream);
			} finally {
				if (fileStream != null) {
			...
			try {
				...
				if (property != null && PROP_HANDLER.equals(property.key)) {
					...
					if (property.value != null) {
						handlerClassName = property.value;
						handlerClass = Clazz.forName(handlerClassName);

						if (handlerClass == null) {
							Log.error("Properties.init: unknown handler class " + handlerClass);
							throw new ClassNotFoundException();
						}
						if (!loadedhandlerMap.containsKey(handlerClassName)) {
							try {
								PropertiesHandler loadedHandler = (PropertiesHandler) handlerClass.newInstance();
								loadedhandlerMap.put(handlerClassName, loadedHandler);
							...
							handler = (PropertiesHandler) loadedhandlerMap.get(handlerClassName);
						}
					...
					handlerClass = Clazz.forName(loadClass);
					...
					if (Log.isDebug()) {
						Log.debug("Properties.init: No own handler for section" + header + ", checking super handlers", Log.DEBUG_LAYER_FRAMEWORK);
					}
					...
					try {
						...
						throw new RuntimeException("Invalid config file format. Header:" + header);
					}
	 ...
	 * handlers
	 * 
	 * @return all loaded instances of classes that implements PropertiesHandler
	 */
	...
	private static Property readNextProperty(PropertiesInputStream in) throws SectionHeaderFoundException {
		...
		while (c != 1) {
			if (c == '#') {
				...
				SimpleStringBuilder buf = Toolkit.getInstance().createSimpleStringBuilder(64);
					...
					switch (c) {
						case '=':
							if (key == null) {
								...
								buf = Toolkit.getInstance().createSimpleStringBuilder(128);
							} else {
					...
					switch (c) {
						...
						case '\n':
							if (header != null) {
								...
								buf = Toolkit.getInstance().createSimpleStringBuilder(64);
							}
							break;
						default:
							if (depth > 0) {
