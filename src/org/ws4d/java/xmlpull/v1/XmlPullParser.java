 ...
 * //------100-columns-wide------>|
 */
// for license please see accompanying LICENSE.txt file (available also at
// http://www.xmlpull.org/)

package org.ws4d.java.xmlpull.v1;
 ...
 * <li>when FEATURE_PROCESS_DOCDECL is false (this is default and if different value is required necessary must be changed before parsing is started) then parser behaves like XML 1.0 compliant non-validating parser under condition that <em>no DOCDECL is present</em> in XML documents (internal entities can still be defined with defineEntityReplacementText()). This mode of operation is intended <b>for operation in constrained environments</b> such as J2ME.
 * </ul>
 * <p>
 ...
 * <p>
 * The current event state of the parser can be determined by calling the <a href="#getEventType()">getEventType()</a> method. Initially, the parser is in the <a href="#START_DOCUMENT">START_DOCUMENT</a> state.
 * <p>
 * The method <a href="#next()">next()</a> advances the parser to the next event. The int value returned from next determines the current parser state and is identical to the value returned from following calls to getEventType ().
 * <p>
 ...
 * </dl>
 * <p>
 * after first next() or nextToken() (or any other next*() method) has been called, the user application can obtain XML version, standalone and encoding from XML declaration in the following ways:
 * <ul>
 * <li><b>version</b>: getProperty(&quot;<a href="http://xmlpull.org/v1/doc/properties.html#xmldecl-version" >http://xmlpull.org/v1/doc/properties.html#xmldecl-version</a>&quot;) returns String ("1.0") or null if XMLDecl has not been read or if property is not supported
 * <li><b>standalone</b>: getProperty(&quot;<a href="http://xmlpull.org/v1/doc/features.html#xmldecl-standalone" >http://xmlpull.org/v1/doc/features.html#xmldecl-standalone</a>&quot;) returns Boolean: null if there was no standalone declaration or if property is not supported otherwise returns Boolean(true) if standalone="yes" and Boolean(false) when standalone="no"
...
public interface XmlPullParser {
	...
	/**
	 * Signalize that the parser is at the very beginning of the document and
	 * nothing has been read yet. This event type can only be observed by
	 * calling getEvent() before the first call to next(), nextToken, or
	 * nextTag()</a>).
	 * 
	 * @see #next
	 * @see #nextToken
	 ...
	 * href="#nextToken()">nextToken()</a> when a start tag has been read. The
	 * name of start tag is available from getName(), its namespace and prefix
	 * are available from getNamespace() and getPrefix() if <a
	 ...
	 * href="#nextToken()">nextToken()</a> when an end tag has been read. The
	 * name of start tag is available from getName(), its namespace and prefix
	 ...
	 * <p>
	 * <strong>Please note:</strong> <a href="#next()">next()</a> will accumulate multiple events into one TEXT event, skipping IGNORABLE_WHITESPACE, PROCESSING_INSTRUCTION and COMMENT events, In contrast, <a href="#nextToken()">nextToken()</a> will stop reading text when any other event is observed. Also, when the state has been reached by calling next(), the text value will be normalized, whereas getText() will return unnormalized content in the case of nextToken(). This allows to have an exact roundtrip without changing line ends when examining low level events, whereas for high level applications the text is appropriately normalized
	 * 
	 * @see #next
	 * @see #nextToken
	 * @see #getText
	...
	/**
	 * A CDATA section has just been read; this token is available only from
	 * calls to <a href="#nextToken()">nextToken()</a>. A call to next() will
	...
	/**
	 * An entity reference has just been read; this token is available from <a
	 ...
	 * calling getTextt(); otherwise, the user is responsible for resolving the
	 * entity reference. This event type is never returned from next(); next()
	 * will accumulate the replacement text and any other text events to a
	 * single TEXT event.
	 * 
	 * @see #nextToken
	 * @see #getText
	...
	/**
	 * Ignorable whitespace has just been read. This token is available only
	 * from <a href="#nextToken()">nextToken()</a>). For non-validating parsers,
	 * this event is only reported by nextToken() when outside the root element.
	 ...
	 * getText()
	 * <p>
	 * <strong>NOTE:</strong> this is different from calling the isWhitespace() method, since text content may be whitespaces but not ignorable. Ignorable whitespaces are skipped by next() automatically; this event type is never returned by next().
	 * 
	 * @see #nextToken
	 * @see #getText
	 */
	...
	/**
	 * An XML processing instruction declaration has just been read. This event
	 * type is available only via <a href="#nextToken()">nextToken()</a>.
	...
	/**
	 * An XML comment has just been read. This event type is this token is
	 * available via <a href="#nextToken()">nextToken()</a> only; calls to
	 * next() will skip comments automatically. The content of the comment can
	 * be accessed using the getText() method.
	 * 
	 * @see #nextToken
	 * @see #getText
	 */
	...
	/**
	 * An XML document type declaration has just been read. This token is
	 * available from <a href="#nextToken()">nextToken()</a> only. The unparsed
	 * text inside the doctype is available via the getText() method.
	 * 
	 * @see #nextToken
	 * @see #getText
	 */
	 ...
	 * dangerous since malicious applications may alter the array despite it
	 * being final, due to limitations of the Java language.
	 */
	 ...
	 * <p>
	 * <strong>NOTE:</strong> The value cannot be changed during parsing and must be set before parsing.
	 * 
	 * @see #getFeature
	 * @see #setFeature
	 */
	 ...
	 * <p>
	 * <strong>Please note:</strong> If the document type declaration has been ignored, entity references may cause exceptions later in the parsing process. The default value of this feature is false. It cannot be changed during parsing.
	 * 
	 * @see #getFeature
	 * @see #setFeature
	 */
	 ...
	 * will be processed.
	 * <p>
	 * <strong>Please Note:</strong> This feature cannot be changed during parsing. The default value is false.
	 * 
	 * @see #getFeature
	 * @see #setFeature
	 */
	...
	/**
	 * Sets the value of a property. The property name is any fully-qualified
	 * URI.
	 * 
	...
	/**
	 * Looks up the value of a property. The property name is any
	 * fully-qualified URI.
	 * <p>
	...
	/**
	 * Sets the input source for parser to the given reader and resets the
	 ...
	 * START_DOCUMENT.
	 * <p>
	 * <strong>NOTE:</strong> If an input encoding string is passed, it MUST be used. Otherwise, if inputEncoding is null, the parser SHOULD try to determine input encoding following XML 1.0 specification (see below). If encoding detection is supported then the following feature <a href="http://xmlpull.org/v1/doc/features.html#detect-encoding" >http://xmlpull.org/v1/doc/features.html#detect-encoding</a> MUST be true, otherwise it must be false.
	 * 
	 ...
	 * value other than null, this value must be returned by this method.
	 ...
	 * After the first call to next if the XML declaration was present this
	 * method will return encoding declared.
	 */
	String getInputEncoding();

	/**
	 * Sets the new value for entity replacement text as defined in <a
	 ...
	 * enabled, there is no need to manually provide the entity replacement
	 * text.
	 * <p>
	 * The motivation for this function is to allow very small implementations of XMLPULL that will work in J2ME environments. Though these implementations may not be able to process the document type declaration, they can still work with known DTDs by using this function.
	 * <p>
	 ...
	 * <p>
	 * <b>Please note:</b> when the parser is on an END_TAG, namespace prefixes that were declared in the corresponding START_TAG are still accessible although they are no longer within scope.
	 */
	 ...
	 * state of the parser.
	 * <p>
	 * If the prefix was not declared within the current scope, null is returned. The default namespace is included in the namespace table and is available via getNamespace (null).
	 * <p>
	 ...
	 * </pre>
	 * <p>
	 * <strong>Please note:</strong> parser implementations may provide more efficient lookup, e.g. using a Hashtable. The 'xml' prefix is bound to "http://www.w3.org/XML/1998/namespace", as defined in the <a href="http://www.w3.org/TR/REC-xml-names/#ns-using">Namespaces in XML</a> specification. Analogous, the 'xmlns' prefix is resolved to <a href="http://www.w3.org/2000/xmlns/">http://www.w3.org/2000/xmlns/</a>
	 * 
	 * @see #getNamespaceCount
	 * @see #getNamespacePrefix
	 * @see #getNamespaceUri
	 ...
	 * The depth is decremented AFTER the end tag event has been observed.
	 * 
	 * <pre>
	 * &lt;!-- outside --&gt;     0
	 * &lt;root>                  1
	...
	/**
	 * Returns a short text describing the current parser state, including its
	 * position, a description of the current event and the data source if
	 * known. This method is especially useful for providing meaningful error
	 ...
	 * thrown.
	 * <p>
	 * <b>Please note:</b> non-validating parsers are not able to distinguish whitespace and ignorable whitespace, except from whitespace outside the root element. Ignorable whitespace is reported as separate event, which is exposed via nextToken only.
	 */
	boolean isWhitespace() throws XmlPullParserException;

	/**
	 * Returns the text content of the current event as String. The value
	 * returned depends on the current event type, for example for TEXT event it
	 * is element content (this is typical case when next() is used). See
	 * description of nextToken() for detailed description of all the possible
	 * returned values for different types of events.
	 * <p>
	 * <strong>NOTE:</strong> in case of ENTITY_REF, this method returns the entity replacement text (or null if not available). This is the only case where getText() and getTextCharacters() return different values.
	 * 
	 * @see #getEventType
	 * @see #next
	 * @see #nextToken
	 ...
	 * <p>
	 * <strong>NOTE:</strong> if FEATURE_REPORT_NAMESPACE_ATTRIBUTES is set then namespace attributes (xmlns:ns='...') must be reported with namespace <a href="http://www.w3.org/2000/xmlns/">http://www.w3.org/2000/xmlns/</a> (visit this URL for description!). The default namespace attribute (xmlns="...") will be reported with empty namespace.
	 * <p>
	 ...
	 * <p>
	 * <strong>NOTE:</strong> attribute value must be normalized (including entity replacement text if PROCESS_DOCDECL is false) as described in <a href="http://www.w3.org/TR/REC-xml#AVNormalize">XML 1.0 section 3.3.3 Attribute-Value Normalization</a>
	 * 
	 * @see #defineEntityReplacementText
	 * @param zero based index of attribute
	 ...
	 * <dd>return characters that are determined to be ignorable white space. If the FEATURE_XML_ROUNDTRIP is enabled all whitespace content outside root element will always reported as IGNORABLE_WHITESPACE otherise rteporting is optional. <br>
	 ...
	 * <dd>if FEATURE_XML_ROUNDTRIP is true return exact PI content ex: 'pi foo' from &lt;?pi foo?> otherwise it may be exact PI content or concatenation of PI target, space and data so for example for &lt;?target data?> string &quot;target data&quot; may be returned if FEATURE_XML_ROUNDTRIP is false.
	 * <dt>COMMENT
	 * <dd>return comment content ex. 'foo bar' from &lt;!--foo bar-->
	 * <dt>ENTITY_REF
	 * <dd>getText() MUST return entity replacement text if PROCESS_DOCDECL is false otherwise getText() MAY return null, additionally getTextCharacters() MUST return entity name (for example 'entity_name' for &amp;entity_name;). <br>
	 * <b>NOTE:</b> this is the only place where value returned from getText() and getTextCharacters() <b>are different</b> <br>
	 * <b>NOTE:</b> it is user responsibility to resolve entity reference if PROCESS_DOCDECL is false and there is no entity replacement text set in defineEntityReplacementText() method (getText() will be null) <br>
	 * <b>NOTE:</b> character entities (ex. &amp;#32;) and standard entities such as &amp;amp; &amp;lt; &amp;gt; &amp;quot; &amp;apos; are reported as well and are <b>not</b> reported as TEXT tokens but as ENTITY_REF tokens! This requirement is added to allow to do roundtrip of XML documents!
	 * <dt>DOCDECL
	 ...
	 * </dl>
	 * <p>
	 * <strong>NOTE:</strong> there is no gurantee that there will only one TEXT or IGNORABLE_WHITESPACE event from nextToken() as parser may chose to deliver element content in multiple tokens (dividing element content into chunks)
	 * <p>
