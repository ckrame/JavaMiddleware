...
/**
 * This class allows matching messages according to their {@link Message#getType() message type} or the {@link Message#getTo() address
 * of the endpoint} they are addressed to.
 * <p>
 * Selection criteria can be specified in a generic way by means of the {@link #setSelectionProperty(int, List)} method. This method allows declaration of expected values for each property in question. A message will match those criteria only if it has matching values for all the given properties.
 * </p>
 */
public class DefaultMessageSelector implements MessageSelector {
	...
	/**
	 * Property denoting a selection based on the target {@link EndpointReference endpoint reference} of the message (i.e. its
	 ...
	 * means of {@link #setSelectionProperty(int, List)} method, the provided {@link List} <code>values</code> is expected to contain {@link EndpointReference} instances rather than {@link URI}s.
	 */
	 ...
	 * (non-Javadoc)
	 * @see
	 * org.ws4d.java.dispatch.MessageSelector#matches(org.ws4d.java.message.
	 * Message)
	 */
	...
	/**
	 * Adds a <em>&quot;selection rule&quot;</em> by specifying a message <code>property</code> and declaring a list of allowed <code>values</code> for this property. Each message, which has a value for this property that
	 ...
	 * property.
	 * <p>
	 * Calling this method more than once with the same <code>property</code> but different <code>values</code> overwrites (i.e. replaces) previously specified <code>values</code> for that property.
	 * </p>
	 * 
	 ...
	 * @throws IllegalArgumentException if <code>propertyType</code> is neither {@link #MESSAGE_TYPE} nor {@link #ENDPOINT_REFERENCE}
	 */
