...
public abstract class NamedObject extends Annotation {
	 ...
	 * </p>
	 * 
	 * @param value <code>true</code> if this object is abstract, <code>false</code> otherwise.
	 */
	...
	public void checkNamespace(NamedObject no) {
		if (no != null && !no.namespaceCheckPerformed) {
			...
			no.namespaceCheckPerformed = true;
			checkSubs(no);
		}
	}
	...
	private void checkSubs(NamedObject no) {
		...
		if (no instanceof Type) {
			...
			it = t.attributeElements();
			while (it.hasNext()) {
				Attribute a = (Attribute) it.next();
				t.checkNamespace(a);
			}

			it = t.attributeElementGroups();
			while (it.hasNext()) {
			...
			if (t instanceof ComplexType) {
				...
				while (it.hasNext()) {
					AnyElement any = (AnyElement) it.next();
					ct.checkNamespace(any);
				}
