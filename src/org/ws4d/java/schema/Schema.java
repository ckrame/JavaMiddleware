...
public class Schema extends Annotation implements Resource {
	...
	private boolean					resolveSubType			= true;
	...
	public static Schema parse(XmlPullParser parser, URI fromUri, CredentialInfo credentialInfo, String targetNamespace, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException, SchemaException {
		Schema schema;
		synchronized (SCHEMA_CACHE) {
		...
		if (schema == null) {
			...
			schema.handleSchema(new ElementParser(parser), fromUri, credentialInfo, loadReferencedFiles, comManId);
	...
	public static Schema parse(URI fromUri, CredentialInfo credentialInfo, String comManId) throws XmlPullParserException, IOException, SchemaException {
		return parse(fromUri, credentialInfo, true, comManId);
	}

	public static Schema parse(URI fromUri, CredentialInfo credentialInfo, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException, SchemaException {
		ResourceLoader rl = JMEDSFramework.getResourceAsStream(fromUri, credentialInfo, comManId);
		InputStream in;
		if (rl != null && (in = rl.getInputStream()) != null) {
			try {
				Schema schema = parse(in, fromUri, credentialInfo, loadReferencedFiles, comManId);
			...
			throw new IOException("Unable to read schema from URI: " + fromUri);
		}
	}

	public static Schema parse(InputStream in, URI fromUri, CredentialInfo credentialInfo, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException, SchemaException {
	...
	public Type getType(QName name) {
		...
		if (XMLSCHEMA_NAMESPACE.equals(namespace)) {
			return SchemaUtil.getSchemaType(name);
		}
		if (targetNamespace.equals(namespace)) {
	...
	/**
	 * Returns the imports to this schema.
	 * 
	 * @return the imports to this schema.
	 */
	...
	private void addType(Type t, String namespace) {
		...
		if (schemaId == SchemaConstants.XSD_COMPLEXTYPE || schemaId == SchemaConstants.XSD_EXTENDEDCOMPLEXCONTENT || schemaId == SchemaConstants.XSD_RESTRICTEDCOMPLEXCONTENT) {
			...
			Iterator attributes = ct.attributeElements();
			while (attributes.hasNext()) {
				Any anyInnerAttribute = (Any) attributes.next();
				if (anyInnerAttribute.getSchemaIdentifier() == SchemaConstants.XSD_ATTRIBUTE) {
					Attribute innerAttribute = (Attribute) anyInnerAttribute;
					Type innerType = innerAttribute.getType();
					if (innerType != null) {
						if (innerType.getName() != null) {
							if (!types.containsKey(innerType.getName())) {
								addType(innerType);
	...
	void addReferenceGroup(Group g) {
		...
		if (!referenceResolver.contains(g) && !elements.containsValue(g)) {
			referenceResolver.add(g);
		}
	}
	...
	private void serialize0(XmlSerializer serializer) throws IOException {
		...
		prefix = serializer.getPrefix(XSI_NAMESPACE, false);
		if (prefix == null) {
		...
		if (includes.size() > 0) {
			...
			while (it.hasNext()) {
				String iLocation = (String) it.next();
				serializer.startTag(XMLSCHEMA_NAMESPACE, SCHEMA_INCLUDE);
				serializer.attribute(null, SCHEMA_LOCATION, iLocation);
				serializer.endTag(XMLSCHEMA_NAMESPACE, SCHEMA_INCLUDE);
			}
		}
		...
		for (Iterator it = groups.values().iterator(); it.hasNext();) {
			...
			if (Log.isDebug()) {
				Log.debug("Serialized Group: " + group + " " + this);
			}
		}
		...
		do {
			...
			modifications = resolveReferences(serializer, alreadySerializedTypes);
			if (resolveSubType) {
				/*
				 * Resolve subtypes
				 */
				...
				LinkedList subTypesForSerialization = new LinkedList();
				...
				while (!subTypes.isEmpty()) {
					...
					if (typeName != null && !types.containsKey(typeName)) {
						...
						/*
						 * TODO Check this! Why we should serialize types which
						 * are part of an other schema?
						 */
						if (targetNamespace != null && targetNamespace.equals(typeName.getNamespace())) {
							subTypesForSerialization.add(type);
						}
					}
				}
				modifications |= resolveReferences(serializer, alreadySerializedTypes);

				for (Iterator it = subTypesForSerialization.iterator(); it.hasNext();) {
					Type type = (Type) it.next();
					if (!alreadySerializedTypes.contains(type)) {
						type.serialize(serializer, this);
						alreadySerializedTypes.add(type);
					}
				}
			}
			...
			// if (Log.isDebug()) {
			// Log.debug("modifications: " + modifications);
			// }
	...
	// Schlichting
	private boolean resolveReferences(XmlSerializer serializer, LinkedList alreadySerializedTypes) throws IOException {
		...
		while (!referenceResolver.isEmpty() || !baseReferenceResolver.isEmpty()) {
			List notCollectedTypes = new LinkedList();
			...
			for (Iterator it = tempRefResolver.iterator(); it.hasNext();) {
				...
				if (o instanceof Element) {
					...
					if (!elements.containsValue(element)) {
						...
						if (t.getName() != null) {
							...
							if (types != null && !types.containsValue(t) && !XMLSCHEMA_NAMESPACE.equals(namespace)) {
								/*
								 * Collect type which where not serialized in
								 * the first step.
								 */
								notCollectedTypes.add(t);
							}
						}
					}
					...
					if (!XMLSCHEMA_NAMESPACE.equals(st.getName().getNamespace())) {
						if (!alreadySerializedTypes.contains(st)) {
							st.serialize(serializer, this);
							alreadySerializedTypes.add(st);
							if (!notCollectedTypes.contains(st)) {
								notCollectedTypes.add(st);
							}
						}
					}
					...
					if (!groups.containsValue(g)) {
						...
						if (Log.isDebug()) {
							Log.debug("Resolve References Group " + g + " " + this);
						}
					...
					Log.warn("Could not resolve reference " + o);
					// removeList.add(o);
				}
				somethingChanged |= addToTempRemoveResolveList(removeList, o);
			}
			...
			for (Iterator it = removeList.iterator(); it.hasNext();) {
				Object o = it.next();
				if (referenceResolver != EmptyStructures.EMPTY_LIST) {
					referenceResolver.remove(o);
				}
				if (baseReferenceResolver != EmptyStructures.EMPTY_LIST) {
					baseReferenceResolver.remove(o);
				}
			}
			removeList.clear();

			// Bugfix 2010-07-15 SSch
			if (!notCollectedTypes.isEmpty() && types == EmptyStructures.EMPTY_MAP) {
				types = new HashMap();
			}
			 ...
			 * the references.
			 */
			for (Iterator it = notCollectedTypes.iterator(); it.hasNext();) {
				Type type = (Type) it.next();
				addType(type);
			}
	 ...
	 * @param element
	 */
	protected boolean addToTempRemoveResolveList(LinkedList removeList, Object o) {
		// if (Log.isDebug()) {
		// Log.debug("Added to remove List " + o);
		// }
	...
	private void resolveAttributeGroups() throws SchemaException {
		if (attributeGroupResolver == null) {
			return;
		}
	...
	private void handleSchema(ElementParser parser, URI fromUri, CredentialInfo credentialInfo, boolean loadReferencedFiles, String comManId) throws XmlPullParserException, IOException, SchemaException {
		...
		while (parser.nextTag() != XmlPullParser.END_TAG && parser.getDepth() == d + 1) {
			...
			if (XMLSCHEMA_NAMESPACE.equals(namespace)) {
				if (StringUtil.equalsIgnoreCase(Element.TAG_ELEMENT, name)) {
					...
					if (fromUri == null) {
						new ElementParser(parser).consume();
						s = DescriptionRepository.getInstance(comManId).getSchema(schemaLocation, credentialInfo, getTargetNamespace());
					} else {
						URI newUri = URI.absolutize(fromUri, schemaLocation);
						s = SchemaUtil.includeOrImportSchema(parser, newUri, credentialInfo, loadReferencedFiles, comManId);
					}
					if (s != null) {
					...
					if (loadReferencedFiles) {
						...
						if (fromUri == null) {
							new ElementParser(parser).consume();
							s = DescriptionRepository.getInstance(comManId).getSchema(schemaLocation, credentialInfo, nNamespace);
						} else {
							URI newUri = URI.absolutize(fromUri, schemaLocation);
							s = SchemaUtil.includeOrImportSchema(parser, newUri, credentialInfo, true, comManId);
