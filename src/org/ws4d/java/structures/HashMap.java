...
public class HashMap {
	...
	protected transient Entry[]	buckets;
	...
	Entry getEntry(final int bucketIndex, final int hash, final Object key) {
		for (Entry entry = buckets[bucketIndex]; entry != null; entry = entry.next) {
			if ((entry.hash == hash) && ((entry.key == key) || (entry.key != null && entry.key.equals(key)))) {
				return entry;
			}
		}
	...
	public String toString() {
		...
		SimpleStringBuilder sb = Toolkit.getInstance().createSimpleStringBuilder(32 * size);
		sb.append("{");
	...
	public static class Entry {
		...
		public String toString() {
			return Toolkit.getInstance().createSimpleStringBuilder(getKey().toString()).append('=').append(getValue()).toString();
		}
	}
